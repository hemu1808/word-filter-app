...
app = FastAPI(
    ...
    description="API for filtering words by letters and size with concurrent processing",
    version="2.0.0"
)
...
async def load_words_concurrent():
    ...
    def read_and_process_file(filename: str):
        ...
        try:
            start_time = time.time()
            logger.info(f"Starting to load words from {filename}")
            ...
            with open(filename, "r", encoding="utf-8") as file:
                ...
                while True:
                    ...
                    if chunks_processed % 10 == 0:  # Log every 10 chunks
                        logger.debug(f"Processed {chunks_processed} chunks, {len(words)} words so far")
            ...
            logger.info(
                f"Successfully loaded {len(words):,} words in {load_time:.2f}s",
                extra={"extra_fields": {
                    "total_words": len(words),
            ...
            logger.warning(f"Word file {filename} not found. Using sample words.")
            sample_words = [
            ...
            logger.error(f"Error loading words from {filename}: {e}", exc_info=True)
            raise
...
async def startup_event():
    ...
    try:
        ...
        logger.info("Application startup completed successfully", 
                   extra={"extra_fields": {"words_loaded": len(words_list)}})
    except Exception as e:
